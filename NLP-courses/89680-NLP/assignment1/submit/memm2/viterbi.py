...
def viterbi_hmm(sentence_str, train_data):
    ...
    if len(sentence) == 1:
        y = np.argmin(first_word_state_probabilities)
        return [(sentence[0], states[y])]
    ...
    V_current = second_word_state_probabilities
    V_prev = V_current
    ...
    return zip(sentence, ret)

def viterbi_hmm_with_prune(sentence_str, train_data):
    # transition from start, start
    start = config.start
    states = list(train_data.pos_items.keys())
    T = len(states)
