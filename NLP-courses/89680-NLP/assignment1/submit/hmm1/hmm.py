...
def getLogEs(word, train_data):
    ...
    regex_matched = match_against_registered_regexs(word) 
    if len(regex_matched) == 0:
        regex_matched = ['unk']
    result_array = np.zeros(T)

    for regex_name in regex_matched:
        vec =  train_data.word_counts['^'+regex_name] 
        vec_weight = vec.sum() / 1000 + 0.01
        vec = vec / vec_weight
        result_array += vec
